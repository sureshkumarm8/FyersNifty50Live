...
const UnifiedAutoTrade: React.FC<UnifiedAutoTradeProps> = ({ credentials, niftyLtp, historyLog, pivots }) => {
  ...
  const panelProps = { credentials, niftyLtp, historyLog, pivots, tradingMode: mode };
  ...
  return (
    <div className="h-full overflow-y-auto bg-slate-950 text-slate-200">
      <div className="mx-auto max-w-6xl space-y-4 p-4 sm:p-6">
        ...
        {/* Keyed so switching tabs remounts cleanly and a crash in one strategy
            can never take the other down with it. */}
        <ErrorBoundary key={tab} label={`autotrade-${tab}`}>
          {tab === 'sniper' ? <SniperPanel {...panelProps} /> : <MomentumPanel {...panelProps} />}
        </ErrorBoundary>
      </div>
    </div>
  );
