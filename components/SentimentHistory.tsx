...
export const SentimentHistory: React.FC<SentimentHistoryProps> = ({ history, apiKey }) => {
  ...
  const [messages, setMessages] = useState<{role: 'user'|'model', text: string}[]>([
      { role: 'model', text: 'I am ready to analyze this history log. I know exactly how these metrics are calculated. What would you like to know?' }
  ]);
      ...
      {isAiOpen && (
          <div className="flex-1 w-full sm:w-[45%] glass-panel rounded-2xl flex flex-col overflow-hidden animate-in slide-in-from-right duration-300 border-l border-white/10">
               <div className="p-4 bg-indigo-900/20 border-b border-indigo-500/20 flex items-center justify-between">
                   ...
                   <button onClick={() => setIsAiOpen(false)} className="text-slate-500 hover:text-white"><X size={16}/></button>
               </div>
               
               <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar bg-slate-950/50">
                   {messages.map((m, idx) => (
                       <div key={idx} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                           <div className={`p-3 rounded-xl text-xs sm:text-sm max-w-[85%] leading-relaxed ${m.role === 'user' ? 'bg-indigo-600 text-white rounded-br-none' : 'bg-slate-800 text-slate-300 rounded-bl-none border border-white/5'}`}>
