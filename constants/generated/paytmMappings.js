...
function getNifty50SecurityIds() {
  return Object.values(PAYTM_NIFTY50_MAP).map((s) => s.security_id);
}
function getOptionSecurityIds(expiryDate, atmStrike, strikeRange = 20) {
  ...
  return options.filter((opt) => opt.strike >= minStrike && opt.strike <= maxStrike).map((opt) => opt.security_id);
}
function getAvailableExpiries() {
  return Object.keys(PAYTM_NIFTY_OPTIONS).sort();
}
function getUpcomingExpiry() {
...
export {
  ...
  getNifty50SecurityIds,
  getOptionSecurityIds,
  getUpcomingExpiry
};
